...
//
// FT_ListDevices标志(与FT_OpenEx标志联合使用
//
...
//
// FT232H CBUS选项EEPROM值
//

#define FT_232H_CBUS_TRISTATE			0x00	//	三态
#define FT_232H_CBUS_RXLED				0x01	//	Rx LED
#define FT_232H_CBUS_TXLED				0x02	//	Tx LED
#define FT_232H_CBUS_TXRXLED			0x03	//	Tx和Rx LED
#define FT_232H_CBUS_PWREN				0x04	//	权力使
#define FT_232H_CBUS_SLEEP				0x05	//	睡眠
#define FT_232H_CBUS_DRIVE_0			0x06	//	驱动引脚到逻辑0
#define FT_232H_CBUS_DRIVE_1			0x07	//	驱动引脚到逻辑1
#define FT_232H_CBUS_IOMODE				0x08	//	IO模式为CBUS位bang
#define FT_232H_CBUS_TXDEN				0x09	//	Tx数据使能
#define FT_232H_CBUS_CLK30				0x0A	//	30 mhz时钟
#define FT_232H_CBUS_CLK15				0x0B	//	15MHz 时钟
#define FT_232H_CBUS_CLK7_5				0x0C	//	7.5MHz 时钟
...
extern "C" {
	...
	typedef struct ft_program_data {
		...
		DWORD Signature1;			// Header:必须是0x00000000 
		DWORD Signature2;			// Header:必须是0xffffffff
		DWORD Version;				// Header - FT_PROGRAM_DATA版本
			// 0 =原始数据
			// 1 = FT2232C扩展名
			// 2 = FT232R扩展名
			// 3 = FT2232H扩展名
		...
		char *Description;			// USB HS串行转换器
		char *SerialNumber;			// “FT000001”，如果是固定的，则为NULL
		WORD MaxPower;				// 0 <最大功率<= 500
		WORD PnP;					// 0 =禁用，1 =启用
		WORD SelfPowered;			// 0 =总线供电，1 =自供电
		WORD RemoteWakeup;			// 0 =不支持，1 =支持
		//
		// Rev4 (FT232B) extensions
		//
		UCHAR Rev4;					// 若Rev4芯片为非零，否则为零
		UCHAR IsoIn;				// 如果in端点是同步的，则为非零
		UCHAR IsoOut;				//如果输出端点是同步的，则为非零
		UCHAR PullDownEnable;		// 如果下拉启用，则非零
		UCHAR SerNumEnable;			//如果使用序列号，则为非零
		UCHAR USBVersionEnable;		// 如果芯片使用USBVersion，则为非零
		WORD USBVersion;			// BCD (0x0200 => USB2)
		//
		// Rev 5 (FT2232) extensions
		//
		UCHAR Rev5;					//若Rev5芯片为非零，否则为零
		UCHAR IsoInA;				// 如果in端点是同步的，则为非零
		UCHAR IsoInB;				// 如果in端点是同步的，则为非零
		UCHAR IsoOutA;				// 如果输出端点是同步的，则为非零
		UCHAR IsoOutB;				//如果输出端点是同步的，则为非零
		UCHAR PullDownEnable5;		// 如果下拉启用，则非零
		UCHAR SerNumEnable5;		// 如果使用序列号，则为非零
		UCHAR USBVersionEnable5;	//如果芯片使用USBVersion，则为非零
		WORD USBVersion5;			// BCD (0x0200 => USB2)
		UCHAR AIsHighCurrent;		// 非零if接口为大电流
		UCHAR BIsHighCurrent;		//非零if接口为大电流
		UCHAR IFAIsFifo;			// 如果接口是245 FIFO，则为非零
		UCHAR IFAIsFifoTar;			// 如果interface为245 FIFO CPU target，则为非零
