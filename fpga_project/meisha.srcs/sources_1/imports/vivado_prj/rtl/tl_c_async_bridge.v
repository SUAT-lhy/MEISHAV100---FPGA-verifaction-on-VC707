...
module tl_c_async_bridge #(
		...
		,parameter WIDTH_PAYLOAD=	3 + WIDTH_PARAM+ WIDTH_SIZE + WIDTH_SOURCE + WIDTH_ADDRESS + WIDTH_DATA + 1		// Bits per beat
		// }}}
	) (
	...
	async_fifo #(
		...
		.i_wr((S_VALID)),
		.i_wr_data({ s_opcode, s_param, s_size, s_source, s_address, s_data, s_corrupt }),
		.o_wr_full(w_full),
		//
		.i_rclk(M_CLK),		
		.i_rd_reset_n(M_ARESETN),
		.i_rd(M_READY),
		.o_rd_data({ m_opcode, m_param, m_size, m_source, m_address, m_data, m_corrupt }),
		.o_rd_empty(w_empty)
		// }}}
	);
