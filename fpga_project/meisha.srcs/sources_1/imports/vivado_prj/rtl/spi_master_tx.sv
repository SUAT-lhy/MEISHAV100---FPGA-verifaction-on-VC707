...
(
  ...
  assign sdo_valid_oen = (tx_NS != TRANSMIT);
  ...
  always_ff @(posedge clk, negedge rstn)
    begin
      if (~rstn) begin
        ...
        sdo_oen[0]  <= sdo_valid_oen || (en_quad_in == 1'b0);  // standard mode, used for input
        sdo_oen[1]  <= sdo_valid_oen;
        sdo_oen[2]  <= sdo_valid_oen || (en_quad_in == 1'b0);  // standard mode, unused
        sdo_oen[3]  <= sdo_valid_oen || (en_quad_in == 1'b0);  // standard mode, unused
      end
    end
