...
`endif
	...
	wire [LGFIFO:0]	rd_wgray, wr_rgray;
	
	wire	[LGFIFO:0]		next_rd_addr, next_wr_addr;
	wire	[LGFIFO:0]		next_rd_addr_rshift_1, next_wr_addr_rshift_1;

	wire	[LGFIFO:0] rgray, wgray;
	wire wgray_reg_en, rgray_reg_en;

	(* ASYNC_REG = "TRUE" *) reg [(LGFIFO+1)*(NFF-1)-1:0] rgray_cross, wgray_cross;
	wire				wclk;
	...
	`ifndef SYNTHESIS
		initial begin
			if(NFF != 2) begin
            	$fatal("Fatal: NFF is illegal, need set be 2 (instance %m)");
			end
		end
	...
	assign next_wr_addr_rshift_1 = next_wr_addr >> 1;
	assign wgray_reg_en = i_wr && !o_wr_full;
	always @(posedge wclk or negedge i_wr_reset_n) begin
		if (!i_wr_reset_n)
		begin
			wr_addr <= 0;
		end else if (wgray_reg_en)
		begin
	...
	genvar i;
	generate
        for (i = 0; i <= MSB; i = i + 1) begin : wgray_gen
			AsyncResetReg #(.RESET_VALUE(0), .DELAY_CYCLE(1)) wgray_reg ( 
				.rst(~i_wr_reset_n),
				.clk(wclk),
				.en(wgray_reg_en),
				.d(next_wr_addr[i] ^ next_wr_addr_rshift_1[i]),
				.q(wgray[i])
			);
		end
	endgenerate
	...
	assign next_rd_addr_rshift_1 = next_rd_addr >> 1;
	assign  rgray_reg_en = lcl_read && !lcl_rd_empty;
	always @(posedge i_rclk or negedge i_rd_reset_n)
	if (!i_rd_reset_n)
	begin
		rd_addr <= 0;
	end else if (rgray_reg_en)
	begin
	...
	// }}}
	generate
        for (i = 0; i <= MSB; i = i + 1) begin : rgray_gen
			AsyncResetReg #(.RESET_VALUE(0), .DELAY_CYCLE(1)) rgray_reg ( 
				.rst(~i_rd_reset_n),
				.clk(i_rclk),
				.en(rgray_reg_en),
				.d(next_rd_addr[i] ^ next_rd_addr_rshift_1[i]),
				.q(rgray[i])
			);
		end
	endgenerate
	...
	generate
        for (i = 0; i <= MSB; i = i + 1) begin : wr_rgray_gen
			AsyncResetReg #(.RESET_VALUE(0), .DELAY_CYCLE(1)) wr_rgray_reg ( 
				.rst(~i_wr_reset_n),
				.clk(wclk),
				.en(1'b1),
				.d(rgray_cross[i]),
	...
	generate
        for (i = 0; i <= MSB; i = i + 1) begin : rd_wgray_gen
			AsyncResetReg #(.RESET_VALUE(0), .DELAY_CYCLE(1)) rd_wgray_reg ( 
				.rst(~i_rd_reset_n),
				.clk(i_rclk),
				.en(1'b1),
				.d(wgray_cross[i]),
